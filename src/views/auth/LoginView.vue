...
<template>
  <form class="panel space-y-5 p-6" @submit.prevent="submit">
    <div>
      ...
      <p class="mt-1 text-xs text-slate-500">JWT · RBAC 四级权限 · 国密 SM4 地理脱敏 · 通量 / 运维 / 决策</p>
    </div>
    ...
    <p v-if="errorMsg" class="rounded-md border border-rose-300 bg-rose-50 px-3 py-2 text-xs text-rose-700">
      {{ errorMsg }}
    </p>
    ...
    <div class="grid grid-cols-2 gap-2">
      <button
        ...
        :key="d.username"
        type="button"
        class="rounded border border-emerald-100 px-2 py-1 text-[11px] text-slate-500 hover:border-flux-300 hover:text-flux-800"
