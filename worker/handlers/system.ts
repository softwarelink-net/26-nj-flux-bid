import { type Env, FALLBACK_CONFIGS, error, json, parseBody, queryAll, requireAuth, runSql, writeAudit } from '../lib'

export async function handleConfigs(request: Request, env: Env) {
  const auth = await requireAuth(request, env)
  if (auth.error) return auth.error
  let rows = await queryAll(env, 'SELECT * FROM njflux_system_configs ORDER BY category, config_key')
...
export async function handleUpdateConfig(request: Request, env: Env) {
  ...
  if (
    ...
    auth.user.role !== 'ROLE_FLUX_SCIENTIST' &&
    auth.user.role !== 'ROLE_DECISION_MAKER'
  ) {
    return error('仅系统总管、通量科学家或决策研判长可调整系统参数', 403)
  }
  ...
  await runSql(
    env,
    'UPDATE njflux_system_configs SET config_value = ?, updated_at = CURRENT_TIMESTAMP WHERE config_key = ?',
...
export async function handleAuditLogs(request: Request, env: Env) {
  ...
  if (auth.error) return auth.error
  const rows = await queryAll(env, 'SELECT * FROM njflux_audit_logs ORDER BY created_at DESC LIMIT 100')
  return json({ success: true, data: rows || [] })
}
