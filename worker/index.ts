 ...
 * 多站点 R2 静态托管 + 26-nj-flux-bid (D1 Allworld / R2 26-nj-flux-bid-assets / ASSETS) 主机分流
 */
import type { Env } from './lib'
import { json } from './lib'
import { handleApi } from './router'

const PROJECT_SLUG = '26-nj-flux-bid'
...
function isProjectHost(hostname: string, env: Env) {
  ...
  const explicitProjectHosts = new Set(['26-nj-flux-bid.softwarelink.net', `${slug}.${root}`])
...
<head>
  ...
  <style>
    :root { --ink:#064e3b; --muted:#047857; --bg:#ecfdf5; }
    body { margin:0; font-family:"Segoe UI",system-ui,sans-serif; color:var(--ink);
      background: radial-gradient(1000px 500px at 0% 0%,#d1fae5,transparent 55%),
                  radial-gradient(800px 400px at 100% 0%,#a5f3fc,transparent 50%), var(--bg);
      min-height:100vh; }
    ...
    code { background:#d1fae5; padding:.15em .4em; border-radius:4px; font-size:.92em; }
    a { color:#0e7490; }
  </style>
</head>
<body>
  <main>
    ...
    <ul>
      ...
      <li>南京师大高寒生态通量：<a href="https://26-nj-flux-bid.softwarelink.net/">26-nj-flux-bid.softwarelink.net</a></li>
    </ul>
  </main>
</body>
</html>`
...
export default {
  async fetch(request: Request, env: Env) {
    ...
    if (url.pathname.startsWith('/api/') && isProjectHost(url.hostname, env)) {
      try {
    ...
    if (isProjectHost(url.hostname, env)) {
      ...
      return new Response('R2 26-nj-flux-bid-assets / ASSETS not configured', { status: 500 })
    }
